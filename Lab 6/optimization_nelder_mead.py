...
def get_centroid(x_1: np.ndarray, x_2: np.ndarray) -> np.ndarray:
    ...
    """
    return (x_1 + x_2) / 2  # Average of two points, so the middle between them.
...
def shrink(simplex: np.ndarray, sigma: float) -> np.ndarray:
    ...
    """
    #https://en.wikipedia.org/wiki/Nelder%E2%80%93Mead_method#Initial_simplex
    # Shrink above this heading
